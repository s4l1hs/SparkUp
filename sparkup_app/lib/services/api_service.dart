...
class ApiService {
  ...
  Future<Map<String, String>> getTopics() async {
    ...
    if (response.statusCode == 200) {
      final data =
          Map<String, dynamic>.from(_decodeResponseBody(response.bodyBytes));
  ...
  Future<List<String>> getUserTopics(String idToken) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
          "Failed to load user topics. Status: ${response.statusCode}, Body: ${response.body}");
    }
  }
  ...
  Future<void> setUserTopics(String idToken, List<String> topics) async {
    ...
    if (response.statusCode != 200) {
      throw Exception(
          "Failed to set user topics. Status: ${response.statusCode}, Body: ${response.body}");
    }
  }

  // Rastgele bilgi kartı getirir
  Future<Map<String, dynamic>> getRandomInfo(String idToken,
      {String? category}) async {
    final uri = Uri.parse(
        "$backendBaseUrl/info/random/${category != null ? '?category=$category' : ''}");
    final response = await http.get(uri, headers: _getAuthHeaders(idToken));
    if (response.statusCode == 200) {
      return _decodeResponseBody(response.bodyBytes);
    } else {
      throw Exception(
          "Failed to load info. Status: ${response.statusCode}, Body: ${response.body}");
    }
  }
  ...
  Future<Map<String, dynamic>> getRandomChallenge(String idToken,
      {String? lang, bool preview = false}) async {
    final uri = Uri.parse(
        "$backendBaseUrl/challenges/random/${lang != null ? '?lang=$lang' : ''}${preview ? (lang != null ? '&preview=true' : '?preview=true') : ''}");
    final resp =
        await http.get(uri, headers: {'Authorization': 'Bearer $idToken'});
    if (resp.statusCode == 200) {
  ...
  Future<Map<String, dynamic>> submitQuizAnswer(
      String idToken, int questionId, int answerIndex) async {
    ...
    final resp = await http.post(
      ...
      body:
          jsonEncode({'question_id': questionId, 'answer_index': answerIndex}),
    );
    if (resp.statusCode == 200) {
      return Map<String, dynamic>.from(jsonDecode(resp.body) as Map);
    } else {
  ...
  Future<List<Map<String, dynamic>>> getManualTrueFalse(
      {String? idToken}) async {
    final uri = Uri.parse('$backendBaseUrl/manual/truefalse/');
    final headers =
        idToken != null ? _getAuthHeaders(idToken) : <String, String>{};
  ...
  Future<LeaderboardEntry?> getUserRank(String idToken) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
          'Failed to load user rank. Status: ${response.statusCode}, Body: ${response.body}');
    }
  }

  // YENİ EKLENEN FONKSİYON: Abonelik Güncelleme
  Future<void> updateSubscription(
      String idToken, String level, int durationDays) async {
    ...
    if (response.statusCode != 200) {
      throw Exception(
          "Failed to update subscription. Status: ${response.statusCode}, Body: ${response.body}");
    }
  }

  // new: get localized texts for specific quiz question ids (no limits consumed)
  Future<List<Map<String, dynamic>>> getLocalizedQuizQuestions(
      String idToken, List<int> ids,
      {String? lang}) async {
    ...
    if (resp.statusCode == 200) {
      ...
      if (kDebugMode) {
        print(
            "getLocalizedQuizQuestions: bad request (400) for ids=$idsParam lang=$lang");
      }
      return <Map<String, dynamic>>[];
    } else {
      throw Exception(
          'Failed to load localized quiz questions (${resp.statusCode})');
    }
  }

  // new: get localized text for a specific challenge id (no limits consumed)
  Future<Map<String, dynamic>> getLocalizedChallenge(
      String idToken, int challengeId,
      {String? lang}) async {
    final baseUri =
        Uri.parse("$backendBaseUrl/challenges/$challengeId/localize/");
    final uri = baseUri.replace(queryParameters: {
      if (lang != null) 'lang': lang,
    });
    final resp = await http.get(uri, headers: _getAuthHeaders(idToken));
    if (resp.statusCode == 200) {
      ...
      throw Exception(
          'Failed to load localized challenge (${resp.statusCode})');
    }
  }
}
