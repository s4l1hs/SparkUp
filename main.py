...
# --- 2. VERİ MODELLERİ (Değişiklik yok) ---
...
# --- 3. GÜVENLİK, OTURUMLAR VE KİMLİK DOĞRULAMA (Değişiklik yok) ---
...
# --- 5. API ENDPOINT'LERİ (İÇERİK GETİRME ENDPOINT'LERİ GÜNCELLENDİ) ---
@app.get("/")
...
# KULLANICI İLE İLGİLİ (Değişiklik yok)
...
def get_user_profile(db_user: User = Depends(get_current_user), session: Session = Depends(get_session)):
    ...
    return UserProfile(firebase_uid=db_user.firebase_uid, email=db_user.email, score=user_score.score if user_score else 0, topic_preferences=user_topics, language_code=db_user.language_code, notifications_enabled=db_user.notifications_enabled )
...
def delete_user_account(db_user: User = Depends(get_current_user), session: Session = Depends(get_session)):
    ...
    try: auth.delete_user(user_uid_to_delete)
    except Exception as e: print(f"Firebase user deletion failed. UID: {user_uid_to_delete}, Error: {e}")
    return
...
# İÇERİK İLE İLGİLİ (REVİZE EDİLMİŞ)
...
def get_random_info(db_user: User = Depends(get_current_user), session: Session = Depends(get_session)):
    ...
    # YENİ MANTIK: Eğer kullanıcının göreceği yeni bilgi kalmadıysa, gördüğü tüm bilgileri sıfırla.
    if not unseen_info:
        print(f"User {db_user.id} için tüm bilgiler tüketildi. Döngü yeniden başlatılıyor.")
        session.exec(delete(UserSeenInfo).where(UserSeenInfo.user_id == db_user.id))
        session.commit()
        # Sıfırladıktan sonra tekrar tüm bilgileri çek.
        unseen_info = session.exec(select(DailyInfo).where(DailyInfo.category.in_(user_topics))).all()
        # Eğer veritabanında hiç bilgi yoksa hata ver.
        if not unseen_info: raise HTTPException(status_code=404, detail="No info available in the database for selected topics.")
...
def get_quiz_questions(limit: int = 3, db_user: User = Depends(get_current_user), session: Session = Depends(get_session)):
    ...
    # YENİ MANTIK: Eğer limit kadar cevaplanmamış soru yoksa, cevaplanmış soruları temizle.
    if len(unanswered) < limit:
        print(f"User {db_user.id} için yeterli soru kalmadı. Döngü yeniden başlatılıyor.")
        session.exec(delete(UserAnsweredQuestion).where(UserAnsweredQuestion.user_id == db_user.id))
        session.commit()
        # Temizledikten sonra tekrar tüm soruları çek.
        unanswered = session.exec(select(QuizQuestion).where(QuizQuestion.category.in_(user_topics))).all()
        if len(unanswered) < limit:
    ...
    for q in chosen:
        ...
        # Yanlışlıkla response'a doğru cevabı göndermemek için modeli güncelledim.
...
def submit_quiz_answer(payload: AnswerPayload, db_user: User = Depends(get_current_user), session: Session = Depends(get_session)):
    ...
    # Sadece doğru cevaplarsa ve daha önce cevaplamadıysa puan ve cevap kaydı ekle.
    if is_correct:
...
def get_random_challenge(db_user: User = Depends(get_current_user), session: Session = Depends(get_session)):
    ...
    # YENİ MANTIK: Eğer tamamlanmamış challenge kalmadıysa, tamamlananları sıfırla.
    if not uncompleted:
        print(f"User {db_user.id} için tüm challenge'lar tamamlandı. Döngü yeniden başlatılıyor.")
        session.exec(delete(UserCompletedChallenge).where(UserCompletedChallenge.user_id == db_user.id))
        session.commit()
        # Sıfırladıktan sonra tekrar tümünü çek.
